...
contract("Recipes", async (accounts) => {
    ...
    it('Get users transaction', async () => {
        const [owner, ipfsHash, recipeType, timeCreated, origin] = await recipeInstance.getUserTransactions(0, { from: accounts[3] })
        assert.equal(ipfsHash, 'testHash2')
        assert.equal(origin, 'Philippines')
    })

    it('Not allowing user to view Recipe that is payable and should be bought', async () => {
        const allowed = await recipeInstance.isAllowedToView(1, { from: accounts[4] })
    ...
    it('should widthraw an account balance from contract and increase its current value', async () => {
        await recipeInstance.cashOut({ from: accounts[0] })
        const balance = await recipeInstance.getBalance({from: accounts[0]})
        assert.equal(balance, 0, 'count is not equal with expected count')
    })
});
